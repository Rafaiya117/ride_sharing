...
//             final String amountRaw = activeOffer['amount'] ?? '50.00'; // Default fallback value string
...
//     markAsRead(); // FIXED: Uses the HTTP endpoint when entering the room
...
//         if (!isMe) {
//           markAsRead(); // FIXED: Uses the HTTP endpoint for real-time incoming updates
//         }
//         notifyListeners();
//       } 
//       // FIXED: Catch the raw JSON offer websocket event stream payload perfectly
...
//         // Safely clear out decimals if present (e.g., "50.00" -> "50")
//         final String amountClean = double.tryParse(amountRaw)?.toStringAsFixed(0) ?? amountRaw;
...
//   //       debugPrint("Offer Sent Successfully: ${response.data}");
        
//   //       // Keep your exact redirect workflow and view cleanup logic intact
//   //       if (context.mounted) {
//   //         GoRouter.of(context).push('/payment');
//   //       }
//   //       toggleOfferInput(false);
...
//   //       debugPrint("Server Offer Validation Failure: ${e.response?.data}");
//   //     } else {
//   //       debugPrint("Error processing price negotiation framework: $e");
//   //     }
//   //   }
//   // }
//   Future<void> sendOffer(BuildContext context) async {
...
//     try {
//       // 1. Find the current offer message to extract the active negotiation price
...
//       // FIXED: Construct body to match your API specification requirements exactly
//       final Map<String, dynamic> requestBody = {
//         "amount": amountStr,
...
//         // 2. Remove the old pending message tracker from local UI state array loop
//         _messages.removeWhere((msg) => msg.text.contains('💰'));

//         // 3. Inject updated accepted status entry right at index 0
//         _messages.insert(0, ChatMessage(
//           text: "💰 Price Offer: \$$amountStr (accepted)",
//           time: "Just now",
...
//         // 4. Route separation depending on user role permissions
//         if (!TokenStorage.isDriver && action == 'accept') {
//           // Passenger goes to payment page right away!
//           GoRouter.of(context).push('/payment');
//         }
//       }
//     } catch (e) {
